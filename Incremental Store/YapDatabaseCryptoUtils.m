...
{
    NSAssert(filePath.length > 0, @"Error ");
    ...
    @autoreleasepool {
        ...
        NSData *_Nullable data = [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:filePath]
                                                       options:NSDataReadingMappedAlways
                                                         error:&error];
        if (!data || error) {
            NSLog(@"%@ Couldn't read database file header.", self.logTag);
...
{
    NSAssert(databaseFilePath.length > 0, @"doesDatabaseNeedToBeConverted");

    if (![[NSFileManager defaultManager] fileExistsAtPath:databaseFilePath]) {
        NSLog(@"%@ database file not found.", self.logTag);
        return NO;
    }
    ...
    if (isUnencrypted) {
        NSLog(@"%@ doesDatabaseNeedToBeConverted; legacy database header already decrypted.", self.logTag);
        return NO;
    }
                                 ...
                                 recordSaltBlock:(YapRecordDatabaseSaltBlock)recordSaltBlock
{
    if (![self doesDatabaseNeedToBeConverted:databaseFilePath]) {
        NSLog(@"%@ convertDatabaseIfNecessary: database does not need to be converted.", self.logTag);
        return nil;
    }
...
+ (nullable NSError *)convertDatabase:(NSString *)databaseFilePath
                     databasePassword:(NSData *)databasePassword
                      recordSaltBlock:(YapRecordDatabaseSaltBlock)recordSaltBlock
{
    NSAssert(databaseFilePath.length > 0, @"");
    NSAssert(databasePassword.length > 0, @"");
    NSAssert(recordSaltBlock, @"");

    NSLog(@"%@ convertDatabase.", self.logTag);
    ...
    NSData *saltData;
    {
        ...
        NSAssert(headerData.length >= kSQLCipherSaltLength, @"");
        ...
        // unrecoverable state.
        NSLog(@"%@ convertDatabase: salt extracted.", self.logTag);
        BOOL success = recordSaltBlock(saltData);
        if (!success) {
            NSLog(@"Failed to record salt, aborting conversion");
            return YDBErrorWithDescription(@"Failed to record salt");
        }
    }
    
    NSLog(@"%@ convertDatabase: key spec derived.", self.logTag);
    ...
    int status;
    {
        ...
        if (status != SQLITE_OK) {
            ...
            if (db) {
                NSLog(@"Error opening database: %d %s", status, sqlite3_errmsg(db));
            } else {
    ...
    NSLog(@"%@ convertDatabase: database open.", self.logTag);
    ...
    // This block was derived from [Yapdatabase configureEncryptionForDatabase].
    {
        ...
        if (status != SQLITE_OK) {
            NSLog(@"Error setting SQLCipher key: %d %s", status, sqlite3_errmsg(db));
            return YDBErrorWithDescription(@"Failed to set SQLCipher key");
        }
    }
    
    NSLog(@"%@ convertDatabase: database keyed.", self.logTag);
    ...
    NSLog(@"%@ convertDatabase: database configured.", self.logTag);
    ...
    // match.
    {
        ...
        NSAssert([[self hexadecimalStringForData:saltData] isEqualToString:saltString], @"");
    }
    
    NSLog(@"%@ convertDatabase: salt confirmed.", self.logTag);
#endif
    ...
    // SQLCipher migration
    {
        ...
        NSLog(@"%@ convertDatabase: encrypted header configured.", self.logTag);
        ...
        NSLog(@"%@ convertDatabase: database dirtied.", self.logTag);
        ...
        if (status != SQLITE_OK) {
            NSLog(@"%@ Error forcing checkpoint. status: %d, log: %d, ckpt: %d, error: %s", self.logTag, status, log, ckpt, sqlite3_errmsg(db));
            return YDBErrorWithDescription(@"Error forcing checkpoint.");
        }
        
        NSLog(@"%@ convertDatabase: checkpoint completed.", self.logTag);

        sqlite3_close(db);
        
        NSLog(@"%@ convertDatabase: database closed.", self.logTag);
    }
...
+ (nullable NSString *)executeSingleStringQuery:(NSString *)sql db:(sqlite3 *)db label:(NSString *)label
{
    ...
    if (status != SQLITE_OK) {
        NSLog(@"%@ Error %@: %d, error: %s", self.logTag, label, status, sqlite3_errmsg(db));
        return nil;
    }

    status = sqlite3_step(statement);
    if (status != SQLITE_ROW) {
        NSLog(@"%@ Missing %@: %d, error: %s", self.logTag, label, status, sqlite3_errmsg(db));
        return nil;
    }

    const unsigned char *valueBytes = sqlite3_column_text(statement, 0);
    int valueLength = sqlite3_column_bytes(statement, 0);
    NSAssert(valueLength == kSqliteHeaderLength, @"");
    NSAssert(valueBytes != NULL, @"");
...
{
    NSAssert(passwordData.length > 0, @"");
    NSAssert(saltData.length == kSQLCipherSaltLength, @"");

    NSMutableData *_Nullable derivedKeyData = [NSMutableData dataWithLength:kSQLCipherDerivedKeyLength];
    if (!derivedKeyData) {
        NSAssert(0, @"failed to allocate derivedKeyData");
        return nil;
    }
    ...
    if (result != kCCSuccess) {
        NSLog(@"Error deriving key: %d", result);
        return nil;
    }
...
+ (nullable NSData *)deriveDatabaseKeySpecForPassword:(NSData *)passwordData saltData:(NSData *)saltData
{
    ...
    NSAssert(keySpecData.length == kSQLCipherKeySpecLength, @"");
